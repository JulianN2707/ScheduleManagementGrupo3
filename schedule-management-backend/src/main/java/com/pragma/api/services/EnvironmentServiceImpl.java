...
public class EnvironmentServiceImpl implements IEnvironmentService {
    ...
    public Response<List<EnvironmentDTO>> findAllBuildings(String facultyId) {
        ...
        // ambiente
        if (!this.environmentRepository.existsBy())
        ...
        List<EnvironmentDTO> EnvironmentDTOlist = modelMapper.map(buildings, new TypeToken<List<EnvironmentDTO>>() {
        }.getType());
    ...
    // Metodo para listar los ambientes por id facultad, edificio, mostrando el tipo
    // que deberia ser pasado
    @Override
    ...
    @Override
    public Response<List<EnvironmentDTO>> findAllByTypeAndParentId(EnvironmentTypeEnumeration environmentType,
            Integer parentId) {
        String environmentTypeString = environmentType.toString();
        List<Environment> environmentPage = this.environmentRepository.findAllByTypeAndParentId(parentId,
        ...
        List<EnvironmentDTO> EnvironmentDTOlist = modelMapper.map(environmentPage,
                new TypeToken<List<EnvironmentDTO>>() {
                }.getType());
        response.setStatus(200);
        response.setUserMessage("List of buildings Finded successfully");
        response.setDeveloperMessage("List of buildings Finded successfully");
